    ...
    async def update_pnl(self, market_data: dict, intel, current_time: float) -> Tuple[None, None, List]:
        ...
        for i, (sym, pos) in enumerate(pos_snapshot):
            ...
            if pos['type'] == 'LONG' and price_drop_pct > 0.40:
                logging.warning(f"🛡️ [IronShield] Flash Crash Ignored (LONG): {sym} (Entry: {pos['entry']} -> Curr: {curr_p}, Drop: {price_drop_pct*100:.1f}%)")
                continue
            if pos['type'] == 'SHORT' and price_rise_pct > 0.40:
                logging.warning(f"🛡️ [IronShield] Flash Pump Ignored (SHORT): {sym} (Entry: {pos['entry']} -> Curr: {curr_p}, Rise: {price_rise_pct*100:.1f}%)")
                continue
